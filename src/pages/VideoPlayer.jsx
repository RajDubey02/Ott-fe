...
const VideoPlayer = () => {
  ...
  useEffect(() => {
    if (!hasAccess && isAuthenticated && video) {
      console.log('No access to video, showing subscription modal');
      setShowSubscriptionModal(true);
    }
  }, [hasAccess, isAuthenticated, video]);
...
const checkAccess = async () => {
  if (!isUserLoggedIn()) {
    console.log('User not logged in for access check');
    setHasAccess(false);
    return;
  }

  try {
    const episodeId = video?._id || video?.id || videoId;
...
const handleSubscriptionSuccess = () => {
  ...
  // After successful subscription, re-check access and refresh subscriptions
  checkAccess();
  fetchSubscriptions();
};
